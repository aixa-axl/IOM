...
/**
 * Press release UI label: post field → Theme Settings → English default.
 *
 * @param string     $key     Key suffix (e.g. release_date, region, topic, partner).
 * @param string     $default English default.
 * @param int|string $post_id Optional post ID (defaults to current).
 * @return string
 */
function iom_get_press_label( $key, $default, $post_id = 0 ) {
	$default = (string) $default;
	$key     = sanitize_key( $key );
	...
	if ( function_exists( 'get_field' ) && $post_id ) {
		$from_post = get_field( 'pr_ui_' . $key, $post_id );
		if ( is_string( $from_post ) && '' !== trim( $from_post ) ) {
			return trim( $from_post );
		}
	}
	...
	if ( function_exists( 'get_field' ) ) {
		$from_option = get_field( 'pr_label_' . $key, 'option' );
		if ( is_string( $from_option ) && '' !== trim( $from_option ) ) {
			return trim( $from_option );
		}
	}
