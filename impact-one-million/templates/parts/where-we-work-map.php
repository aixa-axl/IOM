...
$map_fs     = get_template_directory() . '/assets/images/maps';
$map_ver    = (string) filemtime( $map_fs . '/where-we-work-map-base.png' );
$map_base   = $map_dir . '/where-we-work-map-base.png?ver=' . rawurlencode( $map_ver );
$map_2x     = $map_dir . '/where-we-work-map-base@2x.png?ver=' . rawurlencode( $map_ver );
$map_3x     = $map_dir . '/where-we-work-map-base@3x.png?ver=' . rawurlencode( $map_ver );
...
<div class="relative w-full cursor-pointer" data-work-map>
	...
	<?php foreach ( $map_slugs as $slug ) : ?>
		<?php
		$hl         = $map_dir . '/highlight-' . $slug . '.png?ver=' . rawurlencode( $map_ver );
		$hl_2x      = $map_dir . '/highlight-' . $slug . '@2x.png?ver=' . rawurlencode( $map_ver );
		$hl_3x      = $map_dir . '/highlight-' . $slug . '@3x.png?ver=' . rawurlencode( $map_ver );
